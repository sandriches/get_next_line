...
t_list	*get_struct(int fd, t_list **lst)
{
	t_list *rtn_node;
	t_list *iterate;
...
char	*fill_line(char **leftover, char **line)
{
	int		len;
	char	*tmp;
...
int		last_fill_line(char **leftover, char **line)
{
	fill_line(leftover, line);
	return (1);
}

int		read_line(int fd, char **leftover, char **line)
{
